...
/**
 * Reopens scoring (CLOSED to OPEN). No event and no mail: nothing reacts to this transition.
 */
@Slf4j
@Service
@RequiredArgsConstructor
public class ReopenChallengeScoringService {
    ...
    public ChallengeRestDto execute(
        ...
        if (!accountLookup.canOrganize(organizerAccountId)) {
            log.warn("Reopen scoring rejected, account cannot organize: {}", organizerAccountId);
        ...
        if (!challenge.isOwnedBy(organizerAccountId) && !accountLookup.isAdmin(organizerAccountId)) {
            log.warn("Reopen scoring rejected, account {} does not own challenge {}", organizerAccountId, challengeId);
        ...
        log.info("Challenge scoring reopened: {}", challengeId);
