...
| 3 | Status chip's leading icon glyph is `check` (bare checkmark, `material-symbols-outlined`) on both Open and Revealed | `<app-status-tag>` hardcoded `check_circle` (circled check, `mat-icon`) for both states | Flagged as the same "our own explicit choice but not literally layout/size" ambiguity as G1's kicker color; asked the user, who chose to widen scope and match exactly. Changed `status-tag.ts`'s icon ligature from `check_circle` to `check` (and its spec's assertion) — affects every screen using `<app-status-tag>`: History, Challenge Detail, Rivalry Detail, Participant Home. Icon *presence* on both states already matched (re-confirmed: the mockup's "Open"/red chip also carries the icon, contrary to the original B2 finding's phrasing of "missing leading icon" as if only Revealed had one) — only the glyph changed. |
...
2. `npx ng test --watch=false` — 132/133 passing, same pre-existing `error-interceptor.spec.ts`
   failure (one run also showed the pre-existing flaky `qr-code.spec.ts` canvas test failing
   alongside it — already documented elsewhere in this doc as occasionally flaky under jsdom,
   unrelated to `status-tag`/`challenge-card`; a re-run passed clean at 132/133).
3. `npm run lint` — clean.
4. Live check: logged in as organizer (`claude@claude.com`), re-rendered `/challenges` after both
   fixes, re-measured `.plate-tint` (`padding:1px 6px`, `radius:4px`, `font:12px` — all match) and
   the status chip icon (`check`, matches), re-screenshotted against the mockup's History screen at
   1280×900.
